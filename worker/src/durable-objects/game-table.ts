...
export class GameTableDO extends DurableObject<Env> {
  ...
  private migrate(): void {
    ...
    // created before the credits column existed need it added here (see the
    // aborted-column backfill just below for the identical pattern).
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    // Refresh this seat's cached credits on every connect/reconnect (see the
    // file header and refreshSeatCredits' own doc comment) — the other
    // refresh point is right after settlement.
   ...
   * in a single `WHERE id IN (...)` D1 query — the only two call sites are
   * connect/reconnect (fetch()) and immediately after settlement
   * (broadcastSettled()); every other broadcast (plays, bids, ready-ups)
   * reads the cached column instead, so credits never cost a D1 round trip
   * on the hot path. A userId whose `users` row no longer exists (deleted
   * account) simply isn't in the result set — its seat row's last cached
   * value is left untouched, per this feature's deleted-user policy.
   */
